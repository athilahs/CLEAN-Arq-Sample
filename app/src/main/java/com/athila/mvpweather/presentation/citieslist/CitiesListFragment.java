...
public class CitiesListFragment extends BaseFragment
    ...
    @Bind(R.id.cities_list_content)
    View mContent;
    @Bind(R.id.cities_list_empty_view)
    View mEmptyView;
    @Bind(R.id.cities_list_screen_cities_list)
    ...
    public void onCityEditionFinished(City updatedCity) {
        if (mRootView != null && isAdded()) {
            Snackbar.make(mRootView, R.string.message_city_edited, Snackbar.LENGTH_SHORT).show();
        }
    }
    ...
    public void openCityInputDialog() {
        ...
        View dialogLayout = LayoutInflater.from(getActivity()).inflate(R.layout.dialog_add_city, null);
        final ValidatableEditText cityName = ((ValidatableEditText) dialogLayout.findViewById(R.id.dialog_addCity_editText_city_name));
        final ValidatableEditText cityLatitude = ((ValidatableEditText) dialogLayout.findViewById(R.id.dialog_addCity_editText_city_latitude));
        final ValidatableEditText cityLongitude = ((ValidatableEditText) dialogLayout.findViewById(R.id.dialog_addCity_editText_city_longitude));
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (editingCity == null) {
                    // creating new city
                    City newCity = new City(cityName.getText().toString(),
                            Double.parseDouble(cityLatitude.getText().toString()),
                            Double.parseDouble(cityLongitude.getText().toString()));

                    mPresenter.addCity(newCity);

                } else {
                    editingCity.setName(cityName.getText().toString());
                    editingCity.setLatitude(Double.parseDouble(cityLatitude.getText().toString()));
                    editingCity.setLongitude(Double.parseDouble(cityLongitude.getText().toString()));
        ...
        switch (requestCode) {
            case PLACE_PICKER_REQUEST_NEW_CITY:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    City newCity = new City(place.getName().toString(),
    ...
    public void onItemCheckedStateChanged(ActionMode mode, int position, long id, boolean checked) {
        ...
        if (isAdded()) {
            mode.setTitle(getResources().getQuantityString(R.plurals.cities_selected_count, mSelectedCities.size(), mSelectedCities.size()));
        }
    }
}
